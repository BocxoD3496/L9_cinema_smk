...
function getAverageRating($connection, $movieId)
{
  ...
  // Проверяем, успешно ли был подготовлен запрос
  if ($stmt === false) {
        ...
        <div class="container">
          ...
          ?>
                <div class='column is-3'>
                    <div class='card movie-card' data-movie-id='<?php echo $row['movieID']; ?>'>
                        ...
                        <div class='card-content'>
                            <div class='content'>
                                ...
                                <div class="star-rating">
                                    <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <span class="star <?= ($i <= $averageRating) ? 'filled' : '' ?>" data-rating="<?= $i ?>">&#9733;</span>
                                    <?php endfor; ?>
                                </div>
                            </div>
                        </div>
    ...
    document.querySelectorAll('.star').forEach(star => {
        star.addEventListener('click', function() {
            ...
            fetch('rate_movie.php', {
                ...
                .then(data => {
                    // Обновление средней оценки на странице
                    updateStarRating(movieCard, data.averageRating);
                })
                .catch((error) => {
